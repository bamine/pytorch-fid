...
parser.add_argument(
    "--weights-url",
    type=str,
    default=FID_WEIGHTS_URL,
    help="Download URL of InceptionV3 model weights",
)
parser.add_argument(
    "--weights-file",
    type=str,
    default=None,
    help="Local path to InceptionV3 model weights",
)
parser.add_argument(
    "path",
    type=str,
...
def get_activations(
    files, model, batch_size=50, dims=2048, device="cpu", num_workers=1, image_size=256
):
    ...
    transforms = TF.Compose([TF.Resize((image_size, image_size)), TF.ToTensor()])
...
def calculate_fid_given_paths(
    paths, batch_size, device, dims, weights_url, weights_file, num_workers=1
):
    ...
    model = InceptionV3(
        [block_idx], weights_url=weights_url, weights_file=weights_file
    ).to(device)
